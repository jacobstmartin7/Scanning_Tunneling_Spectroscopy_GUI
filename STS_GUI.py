...
class interface(Frame):
    ...
    def init_lines(self):
        ...
        self.line_dataset = np.zeros((len_1d, len_1d, 2,self.z_len+1,2))

    def generate_lines(self):
        self.coords_to_iv.keys()
        for i in range(len(self.one_dim_coords)):
            x = self.one_dim_coords[i]
            for j in range(len(self.one_dim_coords)):
                y = self.one_dim_coords[j]
                coord_str = self.coord_to_str(x, y)
    ...
    def get_dIdV(self, coord_str, z):

        z_data,f_data,dfdz_data, dfdz_norm_data = self.coords_to_iv[coord_str]
        ind = z_data.index(z)
        dfdz = dfdz_data[ind]
        ...
        for z in z_data:
            if z_last != z:
                ...
                for i in range(0, len(x_vals)):
                    ...
                    y = y_vals[i]
                    coord_str = self.coord_to_str(x,y)
                    didv, didv_norm = self.get_dIdV(coord_str,z)
