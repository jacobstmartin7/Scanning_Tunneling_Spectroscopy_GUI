...
class interface(Frame):
    ...
    def parse_ORNL_data(self, raw):
        ...
        for row in str_data:
            ...
            if coord not in self.coords_to_iv.keys():
                ...
                self.coords_to_iv[coord][3].append(
                    didv_val)  # this should be changed once I figure out normalization for ORNL data
        self.didv_max = largest_didv
        self.didv_min = smallest_didv
        self.coord_step = x_data[1] - x_data[0]
    ...
    def generate_lines(self):
        for i in range(len(self.one_dim_coords_x)):
            ...
            for j in range(len(self.one_dim_coords_x)):
    ...
    def seperate_voltages(self, data):
        ...
        for z in z_data:
            if z_last != z:
                ...
                first_ind = ind
                if first_run == True:
                    self.one_dim_coords_x = x_vals[0:int(len(x_vals) ** .5)]
    ...
    def dfdz_image_plot(self):
        ...
        x = self.one_dim_coords_x
        y = self.one_dim_coords_x
    ...
    def plot_didv(self):
        if self.rectangle == False:
            ...
            self.ax4.set_ylabel('dIdv (nA/V)')
        z_data, f_data, didv_vals, didv_norm_vals = self.coords_to_iv[self.coordinate]
        self.ax4.plot(z_data, didv_norm_vals, c='#1f77b4')
        self.band_gap()
        if self.rectangle == False:
            self.canvas.draw()
    ...
    def click_on_image(self, event):
        ...
        if self.unclicked == True:
            ...
            if click_coord == release_coord:
                ...
                click_point = (xdata, ydata)
                image_point = self.nearest_coord(click_point)
                self.circ1 = Circle(image_point, radius=2, color='black')
                self.circ3 = Circle(image_point, radius=2, color='black')
    ...
    def band_gap(self):
        ...
        left_x = [z_data[left_side.index(left_max)],z_data[idx_left[-1]]]
        right_vals = [max_didv*.08,right_max]
        right_x = [z_data[idx_right[0]+len(didv_norm_vals)//2], z_data[right_side.index(right_max)+len(didv_norm_vals)//2]]
        background_line = [max_didv*.08]*2
        background_line_x = [z_data[0], z_data[-1]]
