...
class interface(Frame):
    ...
    def generate_lines(self):
        self.df = pd.DataFrame(np.zeros((len(self.one_dim_coords), len(self.one_dim_coords)))
                               , columns=self.one_dim_coords, index=self.one_dim_coords, dtype='object')
        for x in self.one_dim_coords:
            for y in self.one_dim_coords:
                x_n, y_n = self.nearest_coord((x,y))
                coord_str = self.coord_to_str(x_n, y_n)
                z_vals, f_vals, didv_vals, didv_norm_vals = self.coords_to_iv[coord_str]
                didv_line = []
                f_line = []
                for i in range(len(z_vals)):
                    ...
                    didv = didv_vals[i]
                    didv_coord = (z, didv)
                    didv_line.append(didv_coord)
                    f_coord = (z, f)
                    f_line.append(f_coord)

                self.df.loc[x, y] = [f_line, didv_line]
    ...
    def parse_ORNL_data(self,raw):
        ...
        for row in str_data:
            ...
            coord_str = self.coord_to_str(x_val, y_val)
            if coord_str not in self.coords_to_iv.keys():
                self.coords_to_iv[coord_str] = [[z_val], [didv_val], [didv_val], [didv_val]]
            else:
                self.coords_to_iv[coord_str][0].append(z_val)
                self.coords_to_iv[coord_str][1].append(didv_val)
                self.coords_to_iv[coord_str][2].append(didv_val)
                self.coords_to_iv[coord_str][3].append(didv_val) #this should be changed once I figure out normalization for ORNL data
    ...
    def dfdz_image_plot(self):
        ...
        if self.color_max == 1000:
            ...
            self.im3 = self.ax3.pcolormesh(x, y, didz_norm_grid, shading='nearest', picker=True, vmin=self.color_min, vmax=self.color_max)
            self.im1 = self.ax1.pcolormesh(x, y, didz_grid, shading='nearest', picker=True, vmin=self.color_min, vmax=self.color_max)
        self.ax1.title.set_text('dIdV Image at V = ' + str(self.voltage))
    ...
    def click_on_image(self, event):
        ...
        if self.unclicked == True:
            ...
            if click_coord != release_coord and event.name != 'button_press_event':
                ...
                y_min = min(y_bounds)
                y_max = max(y_bounds)
                line_data = self.df.loc[x_min:x_max, y_min:y_max]
                ar = line_data.to_numpy()
                iv_lines = []
                didv_lines = []
                for row in ar:
                    for cell in row:
